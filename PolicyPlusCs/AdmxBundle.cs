...
{
    private Dictionary<AdmxFile, AdmlFile> SourceFiles = new();
    private Dictionary<string, AdmxFile> Namespaces = new();

    // Temporary lists from ADMX files that haven't been integrated yet
    private List<AdmxCategory> RawCategories = new();
    private List<AdmxProduct> RawProducts = new();
    private List<AdmxPolicy> RawPolicies = new();
    private List<AdmxSupportDefinition> RawSupport = new();

    // Lists that include all items, even those that are children of others
    public Dictionary<string, PolicyPlusCategory> FlatCategories = new();
    public Dictionary<string, PolicyPlusProduct> FlatProducts = new();

    // Lists of top-level items only
    public Dictionary<string, PolicyPlusCategory> Categories = new();
    public Dictionary<string, PolicyPlusProduct> Products = new();
    public Dictionary<string, PolicyPlusPolicy> Policies = new();
    public Dictionary<string, PolicyPlusSupport> SupportDefinitions = new();
    ...
    public IEnumerable<AdmxLoadFailure> LoadFolder(string Path, string LanguageCode)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(Path))
        {
            if (file.ToLowerInvariant().EndsWith(".admx"))
            {
                ...
                if (fail is not null) fails.Add(fail);
            }
        }
        BuildStructures();
        return fails;
    ...
    private AdmxLoadFailure AddSingleAdmx(string AdmxPath, string LanguageCode)
    {
        ...
        if (Namespaces.ContainsKey(admx.AdmxNamespace)) return new AdmxLoadFailure(AdmxLoadFailType.DuplicateNamespace, AdmxPath, admx.AdmxNamespace);
    ...
    private void BuildStructures()
    {
        ...
        foreach (var rawCat in RawCategories)
        {
            ...
            {
                DisplayName = ResolveString(rawCat.DisplayCode, rawCat.DefinedIn),
                DisplayExplanation = ResolveString(rawCat.ExplainCode, rawCat.DefinedIn),
                UniqueID = QualifyName(rawCat.ID, rawCat.DefinedIn),
                RawCategory = rawCat
            };
        ...
        foreach (var rawProduct in RawProducts)
        {
            ...
            {
                DisplayName = ResolveString(rawProduct.DisplayCode, rawProduct.DefinedIn),
                UniqueID = QualifyName(rawProduct.ID, rawProduct.DefinedIn),
        ...
        foreach (var rawSup in RawSupport)
        {
            ...
            if (rawSup.Entries is not null)
            {
                ...
                {
                    var supEntry = new PolicyPlusSupportEntry { RawSupportEntry = rawSupEntry };
        ...
        foreach (var rawPol in RawPolicies)
        {
            ...
            if (!string.IsNullOrEmpty(rawPol.PresentationID)) pol.Presentation = ResolvePresentation(rawPol.PresentationID, rawPol.DefinedIn);
        ...
        foreach (var cat in catIds.Values)
        {
            if (!string.IsNullOrEmpty(cat.RawCategory.ParentID))
            {
                ...
                if (parentCat is null) continue; // In case the parent category doesn't exist
        ...
        foreach (var sup in supIds.Values)
        {
            foreach (var supEntry in sup.Elements)
            {
                ...
                if (supEntry.Product is null) supEntry.SupportDefinition = findSupById(targetId);
            }
        }
        ...
        foreach (var cat in catIds)
        {
            ...
            if (cat.Value.Parent is null) Categories.Add(cat.Key, cat.Value);
        }
        foreach (var product in productIds)
        {
            FlatProducts.Add(product.Key, product.Value);
            if (product.Value.Parent is null) Products.Add(product.Key, product.Value);
        }
    ...
    public string ResolveString(string DisplayCode, AdmxFile Admx)
    {
        ...
        return dict.ContainsKey(stringId) ? dict[stringId] : DisplayCode;
    }
    ...
    public Presentation ResolvePresentation(string DisplayCode, AdmxFile Admx)
    {
        ...
        return dict.ContainsKey(presId) ? dict[presId] : null;
    }
    ...
    private string ResolveRef(string Ref, AdmxFile Admx)
    {
        if (Ref.Contains(":"))
        {
            var parts = Ref.Split(new[] { ":" }, 2, StringSplitOptions.None);
