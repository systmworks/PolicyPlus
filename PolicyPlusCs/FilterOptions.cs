...
namespace PolicyPlus
{
    public partial class FilterOptions
    {
        ...
        public DialogResult PresentDialog(FilterConfiguration Configuration, AdmxBundle Workspace)
        {
            ...
            void addProductsToNodeCollection(IEnumerable<PolicyPlusProduct> Products, TreeNodeCollection Nodes) { foreach (var product in Products) { var node = Nodes.Add(product.DisplayName); ProductNodes.Add(product, node); node.Tag = product; if (product.Children is not null) addProductsToNodeCollection(product.Children, node.Nodes); } }
        ...
        public void PrepareDialog(FilterConfiguration Configuration)
        {
            ...
            else
            {
                ...
                void expandIfNecessary(TreeNode Node) { foreach (TreeNode subnode in Node.Nodes) { expandIfNecessary(subnode); if (subnode.IsExpanded | subnode.Checked != Node.Checked) { Node.Expand(); } } }
