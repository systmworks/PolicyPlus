...
public class PolicyLoader
{
    ...
    public PolicyLoader(PolicyLoaderSource Source, string Argument, bool IsUser)
    {
        ...
        switch (Source)
        {
            ...
            case PolicyLoaderSource.LocalRegistry:
                {
                    var pathParts = Argument.Split(new[] { @"\" }, 2, StringSplitOptions.None);
        ...
        switch (SourceType)
        {
            ...
            case PolicyLoaderSource.NtUserDat:
                ...
                using (var machHive = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Default))
                {
                    ...
                    PInvoke.RegLoadKeyW(new IntPtr(0x80000002L), subkeyName, MainSourcePath); // HKEY_LOCAL_MACHINE
    ...
    public bool Close() // Whether cleanup was successful
    {
        ...
        {
            string subkeyName = MainSourceRegKey.Name.Split(new[] { @"\" }, 2, StringSplitOptions.None)[1]; // Remove the host hive name
            MainSourceRegKey.Dispose();
            return PInvoke.RegUnLoadKeyW(new IntPtr(0x80000002L), subkeyName) == 0;
        }
        return true;
    }
    ...
    public string Save() // Returns human-readable info on what happened
    {
        switch (SourceType)
        {
            case PolicyLoaderSource.LocalGpo:
                {
                    PolFile oldPol = File.Exists(MainSourcePath) ? PolFile.Load(MainSourcePath) : new PolFile();
                    var pol = (PolFile)SourceObject;
                    pol.Save(MainSourcePath);
                    UpdateGptIni();
                    // Figure out whether this edition can handle Group Policy application by itself
                    if (SystemInfo.HasGroupPolicyInfrastructure())
                    {
                        PInvoke.RefreshPolicyEx(!User, 0);
    ...
    public string GetCmtxPath()
    {
        ...
        {
            return Path.Combine(Path.GetDirectoryName(MainSourcePath), "comment.cmtx");
        }
        else
        {
            return "";
    ...
    private void UpdateGptIni()
    {
        ...
        if (File.Exists(GptIniPath))
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    int curVersion = int.Parse(line.Split(new[] { "=" }, 2, StringSplitOptions.None)[1], CultureInfo.InvariantCulture);
            ...
            if (!seenVersion) fGpt.WriteLine("Version=" + 0x10001);
            if (!seenMachExts) fGpt.WriteLine(MachExtensionsLine);
            if (!seenUserExts) fGpt.WriteLine(UserExtensionsLine);
        }
        else
        {
    ...
    public string GetDisplayInfo()
    {
        ...
        return !string.IsNullOrEmpty(OriginalArgument) ? name + " (" + OriginalArgument + ")" : name;
    }
}
